...
@section('content')
    ...
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
        ...
        <div class="mt-4 sm:mt-0 flex gap-x-2">
            <a href="{{ route('admin.transactions.invoice', $order) }}" target="_blank"
            ...
            @if ($order->status !== 'cancelled' && $order->status !== 'completed')
                <form action="{{ route('admin.transactions.cancel', $order) }}" method="POST" class="inline"
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        @foreach ($order->items as $item)
                            <div
                                class="flex items-center gap-x-4 pb-4 border-b border-gray-200 last:border-0 dark:border-neutral-700">
                                @if ($item->product && $item->product->primaryImage)
                                    <img class="size-16 rounded-lg object-cover"
                                        src="{{ Storage::url($item->product->primaryImage->image_path) }}"
                                        alt="{{ $item->product_name }}">
                                @else
                                    <div
                                        class="size-16 rounded-lg bg-gray-100 flex items-center justify-center dark:bg-neutral-700">
                                ...
                                @endif
                                <div class="flex-1">
                                    <h3 class="font-semibold text-gray-800 dark:text-neutral-200">{{ $item->product_name }}
                                    </h3>
                                    @if ($item->variant_name)
                                        <p class="text-sm text-gray-500 dark:text-neutral-500">{{ $item->variant_name }}</p>
                                    @endif
                                    <p class="text-sm text-gray-600 dark:text-neutral-400">Rp
                                        {{ number_format($item->price, 0, ',', '.') }} × {{ $item->quantity }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="font-semibold text-gray-800 dark:text-neutral-200">Rp
                    ...
                    <div class="mt-6 space-y-2">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-gray-800 dark:text-neutral-200">Rp
                        ...
                        @endif
                        <div
                            class="flex justify-between text-base font-semibold pt-2 border-t border-gray-200 dark:border-neutral-700">
                    ...
                    <dl class="space-y-3">
                        ...
                        @if ($order->customer_email)
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-neutral-200">{{ $order->customer_email }}
                                </dd>
                            </div>
                        @endif
                        <div>
                        ...
                        @if ($order->shipping_notes)
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-neutral-200">{{ $order->shipping_notes }}
                                </dd>
                            </div>
                        @endif
                    </dl>
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-neutral-800 dark:border-neutral-700">
                ...
                <div class="p-6">
                    <form action="{{ route('admin.transactions.update-status', $order) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="status" class="block text-sm font-medium mb-2 dark:text-white">Status Saat
                                Ini</label>
                            <div class="py-2 px-3 bg-gray-100 rounded-lg dark:bg-neutral-700">
                                <span
                                    class="text-sm font-medium text-{{ $order->status_color }}-600 dark:text-{{ $order->status_color }}-500">
                                    {{ $order->status_label }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            <label for="status" class="block text-sm font-medium mb-2 dark:text-white">Ubah
                                Status</label>
                        ...
                        <div class="mb-4">
                            <label for="notes" class="block text-sm font-medium mb-2 dark:text-white">Catatan
                                (Opsional)</label>
                        ...
                        <button type="submit"
                            class="w-full py-2 px-3 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700">
                            Update Status
                        </button>
                    </form>
                </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm dark:bg-neutral-800 dark:border-neutral-700">
                ...
                <div class="p-6">
                    @if ($order->courier && $order->tracking_number)
                        <dl class="space-y-3 mb-4">
                            ...
                            <div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900 dark:text-neutral-200">{{ $order->tracking_number }}
                                </dd>
                            </div>
                        </dl>
                    @endif
                    ...
                    <button type="button"
                        ...
                        aria-haspopup="dialog" aria-expanded="false" aria-controls="shipping-modal"
    ...
    <div id="shipping-modal"
        class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto pointer-events-none bg-black/60">
        <div
            class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
            <div
                class="flex flex-col bg-white shadow-sm rounded-xl pointer-events-auto dark:bg-neutral-800 dark:shadow-neutral-700/70">
                ...
                <div class="p-4">
                    <form action="{{ route('admin.transactions.update-shipping', $order) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="tracking_number" class="block text-sm font-medium mb-2 dark:text-white">No.
                                Resi</label>
                        ...
                        <div class="mb-4">
                            <label for="shipping_cost" class="block text-sm font-medium mb-2 dark:text-white">Biaya Kirim
                                (Opsional)</label>
                            <input type="number" id="shipping_cost" name="shipping_cost"
                                value="{{ $order->shipping_cost }}" step="0.01" min="0"
                                class="py-2 px-3 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400"
                                placeholder="0">
                        </div>
